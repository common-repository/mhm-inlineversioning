<?php
/*
Plugin Name: Inline version numbering for WordPress (UNMAINTAINED)
Plugin URI: https://wordpress.org/plugins/mhm-inlineversioning/
Description: Remove query parameters from asset links (JavaScript and CSS) and use inline dot notation. This requires changes to Apache's .htaccess file, which are made automatically when activating the plugin. Based on original code by Dominik Schilling, https://dominikschilling.de/880/.
...
class MHM_Inlineversioning
{
    ...
    public function parse_uri($src)
    {
        ...
        // Check to see if the filename contains a dot-separated version number.
        // If so, return the raw SRC instead of the manipulated version.
        $url_parts = explode('?', basename($src));
        $filename = array_shift($url_parts);
